                                ...
                                IConfiguration _configuration) : IAuthRepo
    {
        ...
        public Task<List<SwiftLineUser>> GetExpiredAccounts()
        {
            var expiredAccounts = _context.SwiftLineUsers
                .Where(x => x.Name == "Anonymous" && !x.IsInQueue && x.DateCreated != default && x.DateCreated.AddDays(1) < DateTime.UtcNow.AddHours(1))
                .ToListAsync();
        ...
        public async Task DeleteExpiredAccount(SwiftLineUser user)
        {
            ...
            var lockedUser = await _context.SwiftLineUsers
                .FromSqlInterpolated($"SELECT * FROM public.\"AspNetUsers\" WHERE \"Id\" = {user.Id} FOR UPDATE")
                .SingleOrDefaultAsync();

            if (lockedUser is null || lockedUser.IsInQueue)
            {
                await transaction.RollbackAsync();
                return;
            }

            await _userManager.DeleteAsync(lockedUser);
            await transaction.CommitAsync();
        }
