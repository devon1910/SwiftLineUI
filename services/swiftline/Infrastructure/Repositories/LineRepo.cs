...
namespace Infrastructure.Repositories
{
    public class LineRepo(SwiftLineDatabaseContext dbContext, IConfiguration _configuration, IEmailsDeliveryRepo emailRepo) : ILineRepo
    {
        ...
        public async Task<bool> MarkUserAsServed(Line line, string status, string leaveQueueReason)
        {
            ...
            try
            {
                // Existing persisted timestamps use UTC+1. Preserve that convention until
                // the versioned UTC data migration runs, while correcting the subtraction.
                var completedAt = DateTime.UtcNow.AddHours(1);
                var startedAt = line.DateStartedBeingAttendedTo != default
                    ? line.DateStartedBeingAttendedTo
                    : completedAt;
                var finalStatus = leaveQueueReason == "Got served" ? "served" : status;
                var timeWaited = Math.Max(0, Math.Round((completedAt - startedAt).TotalMinutes, 2));

                var claimed = await dbContext.Lines
                    .Where(x => x.Id == line.Id && !x.IsAttendedTo)
                    .ExecuteUpdateAsync(update => update
                        .SetProperty(x => x.IsAttendedTo, true)
                        .SetProperty(x => x.DateCompletedBeingAttendedTo, completedAt)
                        .SetProperty(x => x.Status, finalStatus)
                        .SetProperty(x => x.LeaveQueueReason, leaveQueueReason)
                        .SetProperty(x => x.TimeWaited, timeWaited));
