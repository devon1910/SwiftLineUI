...
namespace Infrastructure.Repositories
{
    ...
    public class EventRepo(SwiftLineDatabaseContext dbContext, ILineRepo lineRepo, ISignalRNotifierRepo notifier, IAuthRepo authRepo) : IEventRepo
    {
        ...
        public async Task<AuthRes> JoinEvent(string userId, long eventId)
        {
            ...
            try
            {
                ...
                var eventEntity = await dbContext.Events
                    .FromSqlInterpolated($"SELECT * FROM public.\"Events\" WHERE \"Id\" = {eventId} FOR UPDATE")
                ...
                {
                    return AuthResFailed.CreateFailed("This queue is currently paused.");
                }
                ...
                // Get or create user
                SwiftLineUser? user = string.IsNullOrEmpty(userId)
                    ? null
                    : await dbContext.SwiftLineUsers
                        .FromSqlInterpolated($"SELECT * FROM public.\"AspNetUsers\" WHERE \"Id\" = {userId} FOR UPDATE")
                ...
                if (eventEntity.Capacity > 0 && eventEntity.UsersInQueue >= eventEntity.Capacity)
                {
                    return AuthResFailed.CreateFailed("This queue has reached its capacity.");
                }
        ...
        public async Task<bool> ExitQueue(string userId, long lineMemberId, string adminId = "", int position = -1, string leaveQueueReason = "", long eventId = 0)
        {
            Line? line = dbContext.Lines
                .Where(x => (x.UserId == userId || x.Id == lineMemberId) && !x.IsAttendedTo && (eventId == 0 || x.EventId == eventId))
                .FirstOrDefault();

            if (line is null) return false;

            var wasClaimed = await lineRepo.MarkUserAsServed(
                line,
                adminId != "" ? "served by Admin" : "left",
                leaveQueueReason);
