...
namespace SwiftLine.API
{
    public class Notifier(IHubContext<SwiftLineHub> _hubContext, Lazy<IEventRepo> eventRepo) : ISignalRNotifier
    {
        ...
        public async Task ExitQueue(string userId, long lineMemberId, string adminId = "",int position=-1, string leaveQueueReason = "", long eventId = 0)
        {
            ...
            try
            {
                await eventRepo.Value.ExitQueue(userId, lineMemberId, adminId, position, leaveQueueReason, eventId);
