...
export async function processEmailBatch(request: Request) {
  ...
  if (!lease) return Response.json({ status: true, data: { processed: 0 }, message: "Another delivery run is active." });
  try {
    let processed = 0;
    ...
    return Response.json({ status: true, data: { processed }, message: "Email batch processed." });
