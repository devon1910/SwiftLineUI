...
const EventDetails = ({
  ...
  const getStatusIndicator = () => {
    if (!event.isActive) {
      ...
      return {
        icon: (
          <span className="w-2.5 h-2.5 rounded-full bg-sage-500 mr-2"></span>
        ),
        label: "LIVE",
  ...
  return (
    ...
    >
      {/* Modal Content */}
      <div
        className={`relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl p-6 shadow-2xl animate-fadeIn md:p-8 ${
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2">
            ...
            {/* Close button - now part of the flex container */}
            <button
              onClick={() => setShowDetails(false)}
              className={`rounded-full p-3 text-gray-500 transition-colors hover:text-gray-700 ${
          ...
          <div>
            ...
            <div className="mt-4 space-y-3">
              <p className="flex items-center text-sm">
                ...
                Starts at{" "}
                <span className="font-medium ml-1">{startTime}</span>
              </p>
              <p className="flex items-center text-sm">
                <Clock className="h-4 w-4 mr-2 text-gray-500" />
              ...
              <p className="flex items-center text-sm">
                ...
                Users in Queue:{" "}
                <span className="font-medium ml-1">{event.usersInQueue}</span>
              </p>
              <p className="flex items-center text-sm">
                <Clock className="h-4 w-4 mr-2 text-gray-500" />
                Avg. Wait Time:{" "}
                <span className="font-medium ml-1">
                  {event.averageTime}{" "}
                  {event.averageTime > 1 ? "mins" : "min"}
                </span>
              </p>
              <p className="flex items-center text-sm">
                <FiUserCheck className="h-4 w-4 mr-2 text-gray-500" />
                Staff Serving:{" "}
                <span className="font-medium ml-1">{event.staffCount}</span>
              </p>
              {event.location && (
