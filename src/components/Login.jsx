  ...
  const handleLogin = (e) => {
    ...
    loginUser({ email, password })
      .then((response) => {
        ...
        navigator(location.state?.returnTo || "/", {
          state: {
      ...
      .catch((error) => {
        toast.error(error?.response?.data?.message ?? error?.response?.data?.data?.message ?? "Unable to sign in.");
      });
  };
  const apiUrl = import.meta.env.VITE_API_URL;
  const handleGoogleSignIn = async () => {
    window.location.href = `${apiUrl.replace(/\/+$/, "")}/Auth/LoginWithGoogle`;
   };
